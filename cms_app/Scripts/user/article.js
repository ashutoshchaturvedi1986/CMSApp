$(document).ready(function () {
    $('[id*=rblLanguage]').on("change", function () {
        ...
        if (selected == "Hindi") {
            $(textBox).attr('class', 'form-control hindiFont');
        }
        else {
    ...
    $('#btnSave').click(function () {
        ...
        $('#tbProcessPriority tr').filter(function () {
            ...
            subProcessXml += ProcessCode + '~' + subProcessCode + '~' + subProcessPriority +'~'+desc+ '^';
        })
...
function CreateSubProcess() {
    ...
    $('#sltActivity option:selected').each(function (x) {
        ...
        var $tdsl = $('<td/>').append($sl);

        var $txtDesc = $('<textarea/>', { 'class': 'form-control', 'id': 'Description' + x }).attr('style','height:40px;width:100%;');
        var $tdDesc = $('<td/>').append($txtDesc);
