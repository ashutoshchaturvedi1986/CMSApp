$(document).ready(function () {
    ...
    $("#btnSave,#btnUpdate").click(function () {
        ...
        if (action == 'INSERT') {
            ...
            var supplierlist = $('#sltCompany option:selected').attr('data-child');
            if (supplierlist.toUpperCase().split(',').indexOf(code) > -1) {
                alert('This supplier code is already used.');
                return;
            }
        }

        var prmdata = {
            prmSupplierId: supplierId,
            prmSupplierCode: code,
            prmSupplierName: $('#txtName').val().trim(),
